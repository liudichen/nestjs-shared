...
function attachToBody(options, req, reply, next) {
  ...
  const consumerStream = options.onFile || defaultFileConsumer;
  const body = {};
  const requestFiles = [];
  const mp = req.multipart(
    ...
    function (err) {
      if (!err) {
        ...
        /**文件不放在body中时,将body里的字段删除放在req里 */
        if (
          requestFiles.length &&
          options?.addToBody &&
          !options?.attachFileToBody
        ) {
          const fields = requestFiles.map((ele) => ele.fieldname);
...
export function fastifyMultipart(fastify, options, done) {
  options.limits = {
    ...options.limits,
